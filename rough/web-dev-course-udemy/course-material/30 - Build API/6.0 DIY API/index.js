...
app.get("/filter", (req, res) => {
  ...
  // jokes.forEach((joke) => {
  //   if (joke.jokeType.toLowerCase() == type.toLowerCase()){
...
app.post("/jokes", async (req, res) => {
  try {
    if (!req.body.text || !req.body.type) {
      throw new Error("Please provide both joke text and type");
    }
    const newJoke = {
      id: jokes.length + 1,
    ...
    const foundJoke = jokes.find((joke) => joke.id === newJoke.id);
    ...
    res
      .status(500)
      .send(
        "Server is not responding. Please try again with error message " + error
      );
  }
});
  ...
  try {
    if (!req.body.text || !req.body.type) {
      ...
      res
        .status(500)
        .send(
          "Server is not responding. Please try again with error message " +
            error
        );
    }
    ...
    if (jokeIndex) {
      ...
      throw new Error("Couldn't find the object in array");
...
app.delete("/delete/:id", async (req, res) => {
  try {
    const id = req.params.id;
    const findIndex = jokes.findIndex((joke) => {
      joke.id === parseInt(id);
    });
    if (findIndex > -1) {
...
app.delete("/jokes/all", async (req, res) => {
  try {
      jokes = [];
      res.json('Successfull deleted all items in Jokes array ');
  } catch (error) {
    res
      .status(500)
      .send(
